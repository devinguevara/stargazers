...
class Preprocessing(): 
    ...
    def interpolate(self, x_to_interpolate: [], y_col: str, x_col: str , n_neighbors=3, replace=False):
        ...
        if not x_to_interpolate: 
            raise ValueError("You must put in x values to interpolate in the form of a list or array")
        ...
        df_interpolate = pd.DataFrame({f'{x_col}_x_interpolate': x_to_interpolate.flatten(), f'{y_col}_y_interpolated': y_interpolated.flatten()})
    ...
    def get_redshift_corr(self, col: str): 
        ...
        if not col or col not in self.data.columns: 
            raise ValueError("""You need to make sure that the column you're putting in as an argument actually exists in the dataframe you initialized Preprocessing with""")
